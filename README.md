...
1. **Старт (Фаза 0):** определяет владельца PR, находит имя ментора и, если
   он не отказался, контакт, и, после подтверждения, ассайнится на PR — до
   того, как начнётся долгая проверка кода
2. Собирает контекст PR: метаданные, diff, статус CI — если GitHub
   заблокировал запуск, скилл снимает блокировку сам, иначе статуса CI просто
   нет, — задание варианта, предыдущие комментарии
3. Проверяет по чек-листу из трёх уровней — оформление PR, код, тесты
4. Печатает ментору черновик ревью: инлайн-заметки, сводку, предлагаемый вердикт
5. **После подтверждения** постит ревью и лейблы (ассайн уже сделан в Фазе 0)
...
Скилл сначала останавливается в самом начале (Фаза 0) — подтвердить, что PR
берём, контакт ментора (от него можно отказаться) и ассайн. Дальше доходит до
черновика и останавливается снова: прочитай, поправь формулировки при
...
- Исключение одно: одобрение заблокированного запуска CI скилл делает сам, без
  вопроса — но оно ничего не публикует, только открывает доступ к настоящему
  статусу CI
...
- Для снятия блокировки запуска CI нужен `push` к репозиторию потока; без него
  скилл не ломается — сообщает ментору, что одобрить должен он сам

## Структура

| Файл | Что внутри |
|---|---|
| `skills/review-homework/SKILL.md` | Рабочий процесс из шести фаз (0–4, включая 2.5). Точка входа |
...
| `skills/review-homework/references/gh-recipes.md` | Команды `gh`: «Сбор» читает и снимает блокировку CI; «Постинг» — под подтверждением |
