...
```bash
gh pr view N -R $R --json number,title,author,baseRefName,headRefName,isCrossRepository,changedFiles,additions,deletions,url,headRefOid
```
...
Если печатает `no checks reported` — это не «проверок нет», а «они не
запускались». Проверить, не ждёт ли запуск одобрения:

```bash
gh run list -R $R -c <headRefOid> --json conclusion,databaseId,status
```
...
**Сначала выбрать запуск.** Берём запуск по головному коммиту PR (`headRefOid`
из метаданных Фазы 1), а не по имени ветки: по A1 у всех студентов варианта
ветка называется одинаково, `variant-N`, и фильтр по имени отдаст чужие форки.
Домашний PR по правилу 3 никогда не мержится, поэтому студент остаётся первым
контрибьютором весь курс, и каждый его пуш плодит новый гейтнутый запуск —
точность отбора здесь и важна. **Если по головному коммиту запуска нет —
не одобряем ничего.** В шапку черновика: запуск для головного коммита не найден.
Одобрять соседний нельзя: это запись в чужой PR.

Выбрали — одобряем и ждём:
...
Ненулевой код `gh run watch` здесь означает «CI красный» — упал `eslint` или
`karma`, — а не что команда сломалась. Это результат: его читаем и кладём в
черновик, а не перезапускаем.

После `gh run watch` статус читается заново командой `gh pr checks` из начала
секции, и в черновик идёт он, а не «ждёт одобрения».

Если `approve` ответил `403` — прав не хватает:
...
При падении — лог упавшего шага. Берём по головному коммиту, не по имени ветки:
по A1 она называется `variant-N` у всех студентов варианта, и фильтр по имени
отдаст чужой лог.

```bash
gh run list -R $R -c <headRefOid> --json databaseId,conclusion,status
gh run view <databaseId> -R $R --log-failed
```

Если по головному коммиту запуска нет — лога тоже нет; в черновик идёт факт
«запуск для головного коммита не найден», а не лог соседнего запуска.
