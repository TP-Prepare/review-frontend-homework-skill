...
**Сначала выбрать запуск.** Домашний PR по правилу 3 никогда не мержится,
поэтому студент остаётся первым контрибьютором весь курс и каждый его пуш
создаёт новый гейтнутый запуск — в списке легко окажется несколько
`action_required`. Берём тот, чей `headSha` совпадает с головным коммитом PR
(`headRefOid` из метаданных Фазы 1). Одобрить чужой запуск значит положить в
черновик статус CI по устаревшему коду — ровно тот вред, которым правило 1
оправдывает само исключение.
